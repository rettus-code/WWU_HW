...
(define evaluate (lambda (exp b)
                   (cond
                        ...
                        ((special-form? exp b)) 
                        ((list? exp)(apply-function (evaluate (car exp) b)(map (lambda (new) (evaluate new b)) (cdr exp))))
...
(define evaluate-special-form (lambda (exp b)
                               (cond((equal? 'if (car exp))
                                     (cond((evaluate (cadr exp) b)(caddr exp))
                                          (not (evaluate (cadddr exp) b))
                                          (apply (evaluate(car(cdr exp)) b)(map (lambda (x) b)) (cdr(cadr exp)))))
                                     ((equal? 'cond (car exp))
